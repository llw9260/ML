...
def divide_data(num_client=1, num_local_class=10, dataset_name='emnist', i_seed=0):
    ...
    for cls in config_division.keys():
        ...
        distributions = np.random.dirichlet(np.repeat(0.1, config_division[cls]))
        distributions = distributions / distributions.sum()
        distributions = (np.cumsum(distributions) * num_datapoint).astype(int)[:-1]
        for i_partition in range(config_division[cls]):
            if i_partition == 0:
                config_data[cls][1].append(indexes[:distributions[0]])
            elif i_partition == config_division[cls] - 1:
                config_data[cls][1].append(indexes[distributions[i_partition - 1]:])
            else:
                config_data[cls][1].append(indexes[distributions[i_partition - 1]: distributions[i_partition]])
